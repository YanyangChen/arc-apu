...
// @Controller
// @Scope("session")
// @ACFgFunction(id="APUF001")
// @RequestMapping(value=APUtMapping.APUF001)
// public class APUc002 extends ACFaAppController {
// 
// 	@RequestMapping(value=APUtMapping.APUF001_MAIN, method=RequestMethod.GET)
...
@ACFgFunction(id="APUF001")
@RequestMapping(value=APUtMapping.APUF001)
public class APUc001 extends ACFaAppController {
    ...
    private class Search extends ACFgSearch {
        ...
        public Search() {
          ...
          //SQL to combine two tables to form one grid in browser function
            setCustomSQL("select * from (select a.payment_form_no, a.request_date, a.section_id, a.request_department, "
            		+ "a.supplier_code, a.despatch_date, a.remarks, b.supplier_name " +
                    "from arc_payment_request a, arc_supplier b " +
                    "where (a.supplier_code = b.supplier_code) and (a.section_id = '06') and (a.supplier_code <> 'CASH')) ");
    ...
    public String main(ModelMap model) throws Exception {
       ...
       model.addAttribute("searchpayformno", paymentrequestService.getPaymentRequestByNo());
   //    model.addAttribute("subsectionidselect", SectionService.getAllSubSectionName8());
       ...
       model.addAttribute("getsuppliercode", supplierService.getSupplierPairsExcCash()); 
        return view();
    }
    ...
    @RequestMapping(value=APUtMapping.APUF001_SEARCH_AJAX, method=RequestMethod.POST)
    @ResponseBody
    ...
    @RequestMapping(value=APUtMapping.APUF001_GET_SUPPLIER_NAME_AJAX, method=RequestMethod.POST)
    @ResponseBody
    ...
    @RequestMapping(value=APUtMapping.APUF001_GET_PAYMENTDETAILS_TABLE_AJAX, method=RequestMethod.POST) //get rows of the second grid
    @ResponseBody
   ...
   //    select.setCustomSQL("select * from (select pd.payment_form_no, pd.sequence_no, pd.modified_at, pd.sub_section_id,"
   //                 + " pd.purchase_order_no, pd.invoice_no, pd.programme_no, pd.from_episode_no, "
   //                 + "pd.to_episode_no, pd.particulars, pd.job_description, pd.account_allocation,"
   //                 + " pd.payment_amount, pd.include_in_weekly_reporting, "
   //                 + "pd.include_in_monthly_reporting "
   //                 + "from arc_payment_request pr, arc_payment_details pd "
   //                 + "where pr.payment_form_no = pd.payment_form_no )");
     ...
     @RequestMapping(value=APUtMapping.APUF001_GET_FORM_AJAX, method=RequestMethod.POST)
        @ResponseBody
     ...
     @ACFgTransaction
        @RequestMapping(value=APUtMapping.APUF001_SAVE_AJAX, method=RequestMethod.POST)
        @ResponseBody
